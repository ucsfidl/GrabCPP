...
void setBaseName(char* baseName, char* outFileName, char*syncFileName){
	...
	//Make dir.
	char* holdDir = new char[MAX_PATH];
	int lenName = 0;
	CorStrncpy(holdDir, outFileName, MAX_PATH);
	char* makeDir = new char[MAX_PATH];
	CorStrncpy(makeDir,"mkdir ", MAX_PATH);
	char *token = std::strtok(holdDir, "\\" );
	while (token != NULL) {
		lenName = strlen(token);
		CorStrncat(makeDir, token, MAX_PATH);
		CorStrncat(makeDir, "\\", MAX_PATH);
		token = std::strtok(NULL, "\\");
	}
	CorStrncpy(makeDir, makeDir, strlen(makeDir)-lenName-1);
	system(makeDir);

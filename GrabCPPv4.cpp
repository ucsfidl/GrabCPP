...
static BOOL GetOptions(int argc, char *argv[], char *acqServerName, UINT32 *pAcqDeviceIndex, char *configFileName, char *fileName);
static BOOL GetOptionsFromCommandLine(int argc, char *argv[], char *acqServerName, UINT32 *pAcqDeviceIndex, char *configFileName, char *fileName);
...
int main(int argc, char* argv[])
{
   ...
   bool listenUDP = true;  //change to true when done debugging SAP naming
   int frame = 0;
   bool bView = false;
   ...
   if (!GetOptions(argc, argv, acqServerName, &acqDeviceNumber, configFilename, baseName))
   {
  ...
  file.open(syncFileName);
   ...
   while (listenUDP){
	   ...
	   /*******Method 1: Saving at End of Buffer and restarting********************************************//*
	   ...
	   if (recvlen == 4) {
		   ...
		   if (fSync>0){
			   //if receiving a pos (+) value, snap
			   Xfer->Snap();
			   Xfer->Wait(70);
			   frame++;
		   }else{
			   //else quit listening and save out
			   //save sync file
			   //save tiff images
			   listenUDP = false; 
			   for (int i = 0; i < frame; i++){
				   CorStrncpy(outFileName, baseName, MAX_PATH);
				   CorStrncat(outFileName, to_string(counter + i + 1).c_str(), MAX_PATH);
	   ...
	   /*Medthod 2: Snap and Save at Each UDP*/
	   ...
	   if (recvlen == 4) {
		   ...
		   if (fSync > 0){
			   ...
			   Xfer->Wait(70);
			   CorStrncpy(outFileName, baseName, MAX_PATH);
			   CorStrncat(outFileName, to_string(frame+counter).c_str(), MAX_PATH);
			   CorStrncat(outFileName, ".tiff", MAX_PATH);
			   Buffers->Save(outFileName, "-format tiff", frame, 1);
			   file << syncBuf[frame].c_str() << "\n";
			   frame++;
		   }else{
			   listenUDP = false;
		   }
...
static BOOL GetOptions(int argc, char *argv[], char *acqServerName, UINT32 *pAcqDeviceIndex, char *configFileName, char *baseName)
{
   // Check if arguments were passed
   if (argc > 1)
      return GetOptionsFromCommandLine(argc, argv, acqServerName, pAcqDeviceIndex, configFileName, baseName);
   else
      return GetOptionsFromQuestions(acqServerName, pAcqDeviceIndex, configFileName);
}

static BOOL GetOptionsFromCommandLine(int argc, char *argv[], char *acqServerName, UINT32 *pAcqDeviceIndex, char *configFileName, char *baseName)
{
   ...
   if (argc == 5){
	   CorStrncpy(baseName, argv[4], MAX_PATH);
   }
